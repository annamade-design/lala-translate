...
enum MenuBarView {
    static func menu(target: AppDelegate) -> NSMenu {
        let menu = NSMenu()
        let translate = menu.addItem(withTitle: "Translate Selected Text  \(target.shortcut.configuration.displayName)", action: #selector(AppDelegate.trigger), keyEquivalent: "")
